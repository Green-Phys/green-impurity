...
namespace lsqcpp {
  ...
  public:
    ...
    FiniteDifferencesParameter(const Scalar eps) : _eps(eps) {}

    FiniteDifferencesParameter(const Scalar eps, const int threads) : _eps(eps), _threads(threads) {}

    /// Sets the numerical espilon that is used as step between two sucessive function evaluations.
    /// @param eps numerical epsilon
    void setEpsilon(const Scalar eps) { _eps = eps; }

    /// Returns the numerical epsilon, which is used for jacobian approximation.
    /// @return numerical epsilon
    Scalar epsilon() const { return _eps; }
  ...
  struct ForwardDifferences {
    ...
    void operator()(const Eigen::MatrixBase<I>& xval, const Eigen::MatrixBase<O>& fval, const Objective& objective,
                    const FiniteDifferencesParameter<Scalar>& param, Eigen::MatrixBase<J>& jacobian) const {
      static_assert(Eigen::NumTraits<Scalar>::IsInteger == 0, "Finite differences only supports non-integer scalars.");
...
#pragma omp parallel for num_threads(param.threads())
  ...
  struct DenseSVDSolver {
    ...
    bool operator()(const Eigen::MatrixBase<M>& A, const Eigen::MatrixBase<B>& b, Eigen::MatrixBase<X>& x) const {
  ...
  struct DenseCholeskySolver {
    ...
    bool operator()(const Eigen::MatrixBase<M>& A, const Eigen::MatrixBase<B>& b, Eigen::MatrixBase<X>& x) const {
      ...
      if (!decomp.isPositive()) return false;
  ...
  public:
    ...
    NewtonStepRefiner(const Parameter& param) : _param(param) {}

    /// Refines the given newton step and scales it by a constant factor.
    /// @param step newton step which is scaled.
    template <typename Objective>
    void operator()(const InputVector&, const OutputVector&, const JacobiMatrix&, const GradientVector&, const Objective&,
  ...
  struct BarzilaiBorwein {
    enum class Mode { Direct, Inverse };
  };
  ...
  public:
    ...
    BarzilaiBorweinParameter(const BarzilaiBorwein::Mode mode) : _mode(mode) {}

    BarzilaiBorweinParameter(const Scalar constStep) : _constStep(constStep) {}

    BarzilaiBorweinParameter(const BarzilaiBorwein::Mode mode, const Scalar constStep) : _mode(mode), _constStep(constStep) {}

    /// Sets the BarzilaiBorwein operation mode.
    /// @param mode mode
    void setMode(const BarzilaiBorwein::Mode mode) { _mode = mode; }

    /// Returns the BarzilaiBorwein operation mode.
    /// @return mode
    BarzilaiBorwein::Mode mode() const { return _mode; }

    /// Sets the constant step size, which is used when the refiner was not initialized yet.
    /// @param stepSize constant step size
    void setConstantStepSize(const Scalar stepSize) { _constStep = stepSize; }

    /// Returns he constant step size, which is used when the refiner was not initialized yet.
    /// @return constant step size
    Scalar constantStepSize() const { return _constStep; }
  ...
  public:
    ...
    template <typename Objective>
    void operator()(const InputVector& xval, const OutputVector&, const JacobiMatrix&, const GradientVector&, const Objective&,
                    StepVector&        step) {
  ...
  private:
    ...
    Scalar           directStep(const InputVector& xval, const StepVector& step) const {
    ...
    Scalar inverseStep(const InputVector& xval, const StepVector& step) const {
      const auto sk    = xval - _lastXval;
      const auto yk    = step - _lastStep;
  ...
  public:
    ...
    ArmijoBacktrackingParameter(const Scalar decrease, const Scalar c1, const Scalar minStep, const Scalar maxStep,
                                const Index iterations) :
        _decrease(decrease), _c1(c1), _minStep(minStep), _maxStep(maxStep), _maxIt(iterations) {
    ...
    Scalar backtrackingDecrease() const { return _decrease; }
    ...
    Scalar armijoConstant() const { return _c1; }
  ...
  public:
    ...
    template <typename Objective>
    void operator()(const InputVector& xval, const OutputVector& fval, const JacobiMatrix&, const GradientVector& gradient,
                    const Objective& objective, StepVector& step) const {
      ...
      while ((_param.maximumIterations() <= Index{0} || iterations < _param.maximumIterations()) &&
             stepSize * _param.backtrackingDecrease() >= _param.minimumStepBound() && !armijoCondition) {
  ...
  public:
    ...
    WolfeBacktrackingParameter(const Scalar decrease, const Scalar c1, const Scalar c2, const Scalar minStep,
                               const Scalar maxStep, const Index iterations) :
        _decrease(decrease), _c1(c1), _c2(c2), _minStep(minStep), _maxStep(maxStep), _maxIt(iterations) {
      assert(decrease > static_cast<Scalar>(0));
      assert(decrease < static_cast<Scalar>(1));
      assert(c1 > static_cast<Scalar>(0));
      assert(c1 < static_cast<Scalar>(0.5));
    ...
    Scalar wolfeConstant() const { return _c2; }
  ...
  public:
    ...
    void operator()(const InputVector& xval, const OutputVector& fval, const JacobiMatrix&, const GradientVector& gradient,
      ...
      while ((_param.maximumIterations() <= 0 || iterations < _param.maximumIterations()) &&
             stepSize * _param.backtrackingDecrease() >= _param.minimumStepBound() && !(armijoCondition && wolfeCondition)) {
        stepSize = _param.backtrackingDecrease() * stepSize;
        xvalN    = xval - stepSize * step;
        objective(xvalN, fvalN, jacobianN);
  ...
  public:
    ...
    DoglegMethodParameter(const Scalar initRadius, const Scalar maxRadius, const Scalar radiusEps, const Scalar acceptFitness,
                          const Index iterations) :
        _initialRadius(initRadius), _maxRadius(maxRadius), _radiusEps(radiusEps), _acceptFitness(acceptFitness),
        _maxIt(iterations) {}
    ...
    void setAcceptanceFitness(const Scalar fitness) { _acceptFitness = fitness; }

    /// Returns the minimum fitness value at which a model is accepted.
    /// @return minimum fitness for step acceptance
    Scalar acceptanceFitness() const { return _acceptFitness; }
    ...
    Scalar maximumRadius() const { return _maxRadius; }
    ...
    Scalar initialRadius() const { return _initialRadius; }
  ...
  public:
    ...
    NewtonStepRefiner(const Parameter& param) : _param(param), _radius(param.initialRadius()) {}

    template <typename Objective>
    void operator()(const InputVector& xval, const OutputVector& fval, const JacobiMatrix& jacobian,
                    const GradientVector& gradient, const Objective& objective, StepVector& step) {
      // approximate hessian
      ...
      while (modelFitness < _param.acceptanceFitness() &&
        ...
        // if the model fitness is very good then increase it
          ...
          if (_param.maximumRadius() > 0 && _radius > _param.maximumRadius()) _radius = _param.maximumRadius();
        }
  ...
  private:
    ...
    template <typename Objective>
    Scalar calulateModelFitness(const InputVector& xval, const OutputVector& fval, const GradientVector& gradient,
                                const HessianMatrix& hessian, const StepVector& step, const Objective& objective) const {
      const Scalar error    = fval.squaredNorm() / Scalar{2};
  ...
  struct GradientDescentMethod {
    ...
    bool operator()(const Eigen::MatrixBase<I>&, const Eigen::MatrixBase<O>&, const Eigen::MatrixBase<J>&,
                    const Eigen::MatrixBase<G>& gradient, const Objective&, Eigen::MatrixBase<S>& step) const {
      step = gradient;
      return true;
    }
  };
  ...
  struct GaussNewtonMethod {
    ...
    /// @return true on success, otherwise false
    template <typename I, typename O, typename J, typename G, typename Objective, typename S>
    bool operator()(const Eigen::MatrixBase<I>&, const Eigen::MatrixBase<O>&, const Eigen::MatrixBase<J>& jacobian,
                    const Eigen::MatrixBase<G>& gradient, const Objective&, Eigen::MatrixBase<S>& step) const {
      Solver     solver;
  ...
  public:
    ...
    LevenbergMarquardtParameter(const Scalar initialLambda, const Scalar increase, const Scalar decrease,
                                const Index iterations) :
    ...
    Scalar initialLambda() const { return _initialLambda; }
    ...
    Scalar increase() const { return _increase; }
    ...
    Scalar decrease() const { return _decrease; }
  ...
  public:
    ...
    LevenbergMarquardtMethod(const Parameter& param) : _param(param), _lambda(param.initialLambda()) {}
    ...
    /// @return true on success, otherwise false
    template <typename I, typename O, typename J, typename G, typename Objective, typename S>
    bool operator()(const Eigen::MatrixBase<I>& xval, const Eigen::MatrixBase<O>& fval, const Eigen::MatrixBase<J>& jacobian,
                    const Eigen::MatrixBase<G>& gradient, const Objective& objective, Eigen::MatrixBase<S>& step) {
      using InputVector             = typename Eigen::MatrixBase<I>::PlainMatrix;
      using OutputVector            = typename Eigen::MatrixBase<O>::PlainMatrix;
      ...
      Index              iterations = 0;
      while ((_param.maximumIterations() <= 0 || iterations < _param.maximumIterations()) && errorN > error) {
        A = jacobianSq;
        // add identity matrix
        for (Index i = 0; i < A.rows(); ++i) A(i, i) += _lambda;
  ...
  namespace internal {
    ...
    struct ObjectiveEvaluator<true> {
      template <typename InputVector, typename Objective, typename FiniteDifferences, typename Params, typename OutputVector,
                typename JacobiMatrix>
      void operator()(const InputVector& xval, const Objective& objective, const FiniteDifferences&, const Params&,
                      OutputVector& fval, JacobiMatrix& jacobian) const {
    ...
    struct ObjectiveEvaluator<false> {
      template <typename InputVector, typename Objective, typename FiniteDifferences, typename Params, typename OutputVector,
                typename JacobiMatrix>
      void operator()(const InputVector& xval, const Objective& objective, const FiniteDifferences& finiteDifferences,
                      const Params& param, OutputVector& fval, JacobiMatrix& jacobian) const {
        objective(xval, fval);
  ...
  template <typename _Scalar, int _Inputs, int _Outputs, typename _Objective, typename _StepMethod, typename _RefineMethod,
  ...
  public:
    ...
    using Callback                = std::function<bool(const Index, const InputVector&, const OutputVector&, const JacobiMatrix&,
                                        const GradientVector&, const StepVector&)>;
    ...
    void setThreads(const Index threads) { _finiteDifferencesParam.setThreads(threads); }

    /// Set the difference for gradient estimation with finite differences.
    /// @param eps numerical epsilon
    void setNumericalEpsilon(const Scalar eps) { _finiteDifferencesParam.setNumericalEpsilon(eps); }
    ...
    void setMethodParameters(const StepMethodParameter& param) { _methodParam = param; }

    /// Returns the parameters for the newton step calculation method.
    /// @return step calculation parameters
    const StepMethodParameter& methodParameters() const { return _methodParam; }

    /// Sets the parameters for the newton step refinement method, e.g. ArmijoBacktracking, DoglegMethod, etc.
    /// @param param parameters that should be set.
    void setRefinementParameters(const StepRefinerParameter& param) { _refinerParam = param; }

    /// Returns the parameters for the newton step refinement method.
    /// @return step calculation parameters
    const StepRefinerParameter& refinementParameters() const { return _refinerParam; }
    ...
    void setObjective(const Objective& objective) { _objective = objective; }
    ...
    void setMinimumStepLength(const Scalar steplen) { _minStepLen = steplen; }
    ...
    Result minimize(const InputVector& initialGuess) {
      ...
      Index iterations  = 0;
      while ((_maxIt <= 0 || iterations < _maxIt) && gradLen >= _minGradLen && stepLen >= _minStepLen && error >= _minError &&
             callbackResult) {
        xval -= step;
        objective(xval, fval, jacobian);
        ...
        if (_verbosity > 0) {
          ...
          std::stringstream ss;
          ss << "it=" << std::setfill('0') << std::setw(4) << iterations << std::fixed << std::showpoint << std::setprecision(6)
             << "    steplen=" << stepLen << "    gradlen=" << gradLen;

          if (_verbosity > 1) ss << "    callback=" << (callbackResult ? "true" : "false");

          ss << "    error=" << error;

          if (_verbosity > 2) ss << "    xval=" << makePrettyVector(xval);
          if (_verbosity > 3) ss << "    step=" << makePrettyVector(step);
          if (_verbosity > 4) ss << "    fval=" << makePrettyVector(fval);
      ...
      result.converged  = stepLen < _minStepLen || gradLen < _minGradLen || error < _minError;
  ...
  template <typename Scalar, int Inputs, int Outputs, typename Objective, typename RefineMethod = ConstantStepFactor,
            typename Solver = DenseSVDSolver, typename FiniteDifferencesMethod = CentralDifferences>
  using GaussNewton =
      LeastSquaresAlgorithm<Scalar, Inputs, Outputs, Objective, GaussNewtonMethod<Solver>, RefineMethod, FiniteDifferencesMethod>;

  /// Gauss Newton algorithm with dynamic problem size.
  template <typename Scalar, typename Objective, typename RefineMethod = ConstantStepFactor, typename Solver = DenseSVDSolver,
            typename FiniteDifferencesMethod = CentralDifferences>
  using GaussNewtonX =
      GaussNewton<Scalar, Eigen::Dynamic, Eigen::Dynamic, Objective, RefineMethod, Solver, FiniteDifferencesMethod>;

  /// General Gradient Descent algorithm.
  template <typename Scalar, int Inputs, int Outputs, typename Objective, typename RefineMethod = ConstantStepFactor,
            typename FiniteDifferencesMethod = CentralDifferences>
  using GradientDescent =
      LeastSquaresAlgorithm<Scalar, Inputs, Outputs, Objective, GradientDescentMethod, RefineMethod, FiniteDifferencesMethod>;

  /// Gradient Descent algorithm with dynamic problem size.
  template <typename Scalar, typename Objective, typename RefineMethod = ConstantStepFactor,
            typename FiniteDifferencesMethod = CentralDifferences>
  using GradientDescentX =
      GradientDescent<Scalar, Eigen::Dynamic, Eigen::Dynamic, Objective, RefineMethod, FiniteDifferencesMethod>;

  /// General Levenberg Marquardt algorithm.
  template <typename Scalar, int Inputs, int Outputs, typename Objective, typename Solver = DenseSVDSolver,
            typename FiniteDifferencesMethod = CentralDifferences>
  using LevenbergMarquardt = LeastSquaresAlgorithm<Scalar, Inputs, Outputs, Objective, LevenbergMarquardtMethod<Scalar, Solver>,
                                                   ConstantStepFactor, FiniteDifferencesMethod>;

  template <typename Scalar, typename Objective, typename Solver = DenseSVDSolver,
            typename FiniteDifferencesMethod = CentralDifferences>
  using LevenbergMarquardtX =
      LevenbergMarquardt<Scalar, Eigen::Dynamic, Eigen::Dynamic, Objective, Solver, FiniteDifferencesMethod>;
  ...
  namespace parameter {
    ...
    Eigen::Matrix<typename Eigen::MatrixBase<Derived>::Scalar, 3, 1> encodeRotation(const Eigen::MatrixBase<Derived>& rotation) {
      ...
      if (internal::almostZero(theta)) {
        ...
        return fac * Vector3(rotation(2, 1) - rotation(1, 2), rotation(0, 2) - rotation(2, 0), rotation(1, 0) - rotation(0, 1));
      }
    }
